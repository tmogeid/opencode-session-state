...
export function resolveConfig(options?: Record<string, unknown>): SsmConfig {
	...
	if (typeof options.maxEpisodes === "number")
		config.maxEpisodes = options.maxEpisodes;
	if (typeof options.maxStateTokens === "number")
		config.maxStateTokens = options.maxStateTokens;
	if (typeof options.logging === "string") config.logging = options.logging;
	if (typeof options.storageDir === "string")
		config.storageDir = options.storageDir;
	if (typeof options.injection === "boolean")
		config.injectionEnabled = options.injection;
	if (typeof options.autoSummary === "boolean")
		config.autoSummary = options.autoSummary;
	if (typeof options.summarizerInterval === "number")
		config.summarizerInterval = options.summarizerInterval;
	...
	if (!config.apiKey) {
		...
		if (!config.apiKey) {
			log.error(
				"NVIDIA_API_KEY no configurada. El plugin NO funcionará.\n" +
					"Para usar el modelo default (meta/llama-3.1-8b-instruct):\n" +
					"  1. Obtén una API key en https://build.nvidia.com\n" +
					"  2. Configúrala: export NVIDIA_API_KEY=nvapi-...\n" +
					"  3. Reinicia OpenCode\n" +
					'Alternativa: cambia "model" y "apiBaseUrl" en tu opencode.json',
			);
		}
	}

	log.info(
		`Config resolved: model=${config.model} temperature=${config.temperature} maxEpisodes=${config.maxEpisodes} maxStateTokens=${config.maxStateTokens} injection=${config.injectionEnabled} autoSummary=${config.autoSummary}`,
	);
	return config;
}
