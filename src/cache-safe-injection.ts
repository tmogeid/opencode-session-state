 ...
 *
 * Self-contained copy for the session-state-manager clone: inlines the
 * `isRecord` guard and the minimal message types instead of importing from
 * the omo-slim repo, so the plugin builds standalone.
 */
...
export function createTaggedSyntheticPart(spec: TaggedSyntheticPartSpec): MessagePart {
	return {
		type: "text",
		synthetic: true,
		text: spec.text,
...
export function hasTaggedPart(message: MessageWithParts, metadataKey: string): boolean {
 ...
 */
export function stripTaggedContent(messages: unknown[], metadataKey: string): void {
	for (let i = messages.length - 1; i >= 0; i -= 1) {
		const message = messages[i];
		if (!isMessageWithParts(message)) continue;
		const hadParts = message.parts.length > 0;
		message.parts = message.parts.filter((part) => !isTaggedPart(part, metadataKey));
 ...
 */
export function isVolatileTaggedMessage(message: unknown, metadataKey: string): boolean {
	return (
