  ...
  const log = createLogger({
    ...
    showToast: ctx.client?.tui?.showToast
      ? (msg: string, variant: 'error' | 'warning') =>
          ctx.client.tui.showToast({ body: { message: `[SSM] ${msg}`, variant } })
      : undefined,
  });
  setGlobalLogger(log);
  ...
  return {
    ...
    async 'chat.message'(input, output) {
      try {
        ...
        getLogger().error(`chat.message hook error: ${err instanceof Error ? err.message : String(err)}`);
      }
    },
 ...
 */
function extractMessageText(event: { type: string; properties?: Record<string, unknown> }): string | null {
  try {
